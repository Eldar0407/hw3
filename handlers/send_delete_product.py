...
async def delete_product_callback(callback_query: types):
    ...
    if user_id != admin:
        await callback_query.answer('Только администраторы могут удалять товары!', show_alert=True)
        return
...
def register_send_delete_product(dp: Dispatcher):
    dp.register_message_handler(start_sending, commands=['send_products_delete'])
